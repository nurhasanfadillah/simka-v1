      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <Select value={filterClassId} onValueChange={setFilterClassId}>
            ...
            <SelectContent>
              {classes.map(c => (
                <SelectItem key={c.id} value={String(c.id)}>{c.name} — {c.unitName}</SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      {data && (
        <div className="grid grid-cols-1 gap-4 mb-6">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 hover:shadow-md transition-shadow duration-200">
            ...
            <p className="text-2xl font-bold tabular-nums text-gray-900 mt-1">{data.total} siswa</p>
          </div>
        </div>
      )}
            ...
            {loading ? (
              ...
              data.siswa.map(row => (
                <tr key={row.studentId} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm font-semibold tabular-nums text-red-600">{formatRupiah(row.totalTunggakan)}</td>
